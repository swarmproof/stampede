...
| `population/` | Population Factory, `Agent` + six-state machine, `brain` (heuristic + LLM), `providers` + cost model, `frameworks` (v0.3 — drive the swarm with the user's own LangGraph/CrewAI/callable agent via `BrainPool.framework_brain`; both share a tested `ToolCapture`) |
...
- **Real but experimental:** live LLM brains (`population/brain.py` `LLMBrain` + `providers.py`), routed per-agent by a `BrainPool` so heuristic and live-model agents mix in one swarm (verified live against Ollama). Off the CI blocking path — dry-run is the tested path.
- **Live dashboard (`observer/dashboard.py` + `observer/live.py`):** real WebSocket streaming *during* a run — the engine publishes each agent's lifecycle to a `LiveHub`; `run.serve_live` runs uvicorn + the swarm concurrently (`stampede run --live`). Publishing is observational and never affects determinism. The frontend is a single self-contained HTML page (no build step); a richer Vue UI is a later polish.
