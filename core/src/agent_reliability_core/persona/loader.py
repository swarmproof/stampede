...
from agent_reliability_core.persona.schema import Calibration, Persona, PersonaPack, Temperament
...
def load_pack(name_or_path: str, search_paths: list[Path] | None = None) -> PersonaPack:
    """Load a pack by filesystem path, builtin name (``"core"``), or a name found in
    one of ``search_paths``. Resolution: path → builtin → each search path.

    ``search_paths`` is how a consumer (e.g. stampede's community registry) adds its
    own pack directory without this primitive depending on it."""
    ...
    if not path.is_file():
        for extra in search_paths or []:
            candidate = Path(extra) / f"{name_or_path}.yaml"
            if candidate.is_file():
                path = candidate
                break
    if not path.is_file():
        raise FileNotFoundError(
            f"persona pack {name_or_path!r} not found (builtins: {', '.join(list_builtin_packs())})"
        )
